...
def test_classifier_artifact_rejects_backend_mismatch(monkeypatch, tmp_path: Path) -> None:
    train_path = tmp_path / "train.json"
    calibration_path = tmp_path / "calibration.json"
    artifact_path = tmp_path / "classifier.joblib"
    rows = [
    ...
    monkeypatch.setattr(CalibratedTextClassifier, "_fit", lambda self: None)
    loaded = CalibratedTextClassifier(
        labels=["Shoes", "Clothing"],
        alpha=0.3,
        train_path=tmp_path / "missing-train.json",
    ...
    assert loaded.runtime == "FALLBACK"
    assert loaded.reason == "classifier artifact backend does not match runtime backend"


def test_pipeline_reads_classifier_backend(monkeypatch) -> None:
    monkeypatch.setenv("CLASSIFIER_BACKEND", "embedding")
    monkeypatch.setenv("USE_MOCK_LLM", "true")
    pipeline = ReliabilityPipeline()
    assert pipeline.classifier.backend == "embedding"
